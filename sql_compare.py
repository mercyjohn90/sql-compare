...
def generate_report(result: dict, mode: str, fmt: str, out_path: str, ignore_ws: bool):
    ...
    def mk(title, a, b, fromname, toname):
        # SECURITY: difflib.HtmlDiff escapes diff content but not metadata parameters (fromdesc/todesc) or our f-strings.
        # Must explicitly escape title, fromname, and toname to prevent XSS.
        fromdesc = html.escape(fromname)
        todesc = html.escape(toname)
        title_esc = html.escape(title)
        table = hd.make_table(a.splitlines(), b.splitlines(), fromdesc=fromdesc, todesc=todesc, context=True, numlines=3)
